...
const top_songs = async function (req, res) {
  ...
  const pageSize =
    req.query.page_size != null ? parseInt(req.query.page_size, 10) : 10;
...
const top_albums = async function (req, res) {
  ...
  if (!page) {
    ...
    connection.query(
      `
    SELECT a.title, a.album_id, SUM(s.plays) AS plays
    FROM Albums a
...
const search_songs = async function (req, res) {
  ...
  const danceLow = parseFloat(req.query.danceability_low ?? 0);
  const danceHigh = parseFloat(req.query.danceability_high ?? 1);
  ...
  // replace this with your implementation
  const explicitFilter = explicitParam === "true" ? "TRUE" : "FALSE";
  connection.query(
    `
    SELECT *
    FROM Songs
...
const entrance_songs = async function (req, res) {
  ...
  connection.query(
    ...
    ORDER BY s.valence ASC, s.danceability ASC
    LIMIT $3
    `,
